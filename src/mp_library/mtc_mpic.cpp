...
namespace lrt_mpic
{
  ...
  MtcMPIC::MtcMPIC(const std::string folder_path, const std::string mp_file_name)
  {
    Ndof_ = 7;
    CartDof_ = 6;
    type_ = MPtype::move_to_contact;
    name_ = "";
    index_ = 0;
    sloc_ = 0;
    delta_s_loc_ = 0.1;
  ...
  std::shared_ptr<ManipulationPrimitive> MtcMPIC::clone() const
  {
    return std::make_shared<MtcMPIC>(*this);
  }

  GenericPath *MtcMPIC::getPathParam()
  {
    return &path_;
  }

  setPoint *MtcMPIC::getSetPoint(const double s, const Eigen::VectorXd &q, const Eigen::VectorXd &p, const Eigen::VectorXd &F)
  {
    linearPath(s);
    return &pathSetPnt_;
  }

  setPoint *MtcMPIC::getSetPointGradient(const double s)
  {
    linearPathGradient(s);
    return &pathSetPntGradient_;
  }

  weights *MtcMPIC::getWeights()
  {
    return &wghts_;
  }

  // optionally reduce speed at the beginning and end of the MP
  weights *MtcMPIC::getScaledWeights(const double s, const double s_f)
  {
    wghtsScaled_ = wghts_;
  ...
  constraints *MtcMPIC::getConstraints()
  {
    return &constr_;
  }

  std::string MtcMPIC::getName() { return name_; }

  MPtype *MtcMPIC::getType() { return &type_; }

  bool MtcMPIC::evaluateMP(const double &s)
  {
  ...
  FinishFlag MtcMPIC::evaluateMP(const double &s, const Eigen::VectorXd &q, const Eigen::VectorXd &p, const Eigen::VectorXd &F)
  {
    double s_loc, F_des;
    unsigned index;
    ...
    lrt_mpic::lerp(F_des, path_.F.F_z[index_], path_.F.F_z[index_ + 1], s_loc);
  ...
  double MtcMPIC::getInitialProgressParam() { return path_.s[0]; }
  double MtcMPIC::getTerminationProgressParam() { return path_.s[path_.nSamples - 1]; }

  ReactionStrategy MtcMPIC::getReactionStrategy() { return reaction_; }
  ExceptionStrategy MtcMPIC::getExceptionStrategy() { return exception_; }

  void MtcMPIC::print()
  {
    std::cout << "-------- MP of type " << mpTypeToString(type_) << " named " << name_ << std::endl;
    std::cout << "--- MTC-MP --- " << std::endl;
    ...
    std::cout << "  F_th: " << f_th_ << std::endl;
    std::cout << "  Delta_z: " << delta_z_ << std::endl;
  ...
  bool MtcMPIC::initializeFromFile(const std::string folder_path, const std::string mp_file_name)
  {
    // Load MP YAML file
    mp_node_ = YAML::LoadFile(folder_path + mp_file_name);

    std::cout << "[MtcMPIC::initialize]: MP of type: move_to_contact" << std::endl;
    ...
    {
      std::cout << "[MtcMPIC::initialize]: Invalid basic weight parameter of MP: type MTC-MP" << std::endl;
    }
    ...
    catch (...)
    {
      ...
      std::cout << "[MtcMPIC::initialize]: nSamples of MP: type MTC-MP reading failed!" << std::endl;
    }
    ...
    {
      // std::cout << "[GenericMPIC::initialize]: Invalid transition parameters of MP: type generic MPIC-MP" << std::endl;
      // Pose condition
    ...
    try
    {
      f_th_ = mp_node_["mp/param"]["F_th"].as<std::vector<double>>()[0];
    }
    catch (...)
    {
      f_th_ = 5;
    ...
    try
    {
      delta_z_ = mp_node_["mp/param"]["delta_z"].as<std::vector<double>>()[0];
    }
    catch (...)
    {
      delta_z_ = 0.03;
    ...
    {
      std::cout << "[MtcMPIC::initialize]: Invalid progress parameter s of MP: type MTC-MP" << std::endl;
    }
    ...
    {
      std::cout << "[MtcMPIC::initialize]: Invalid size of MP parameter!" << std::endl;
      return false;
    }
    else
    {
      ...
      double deltaZpersample = delta_z_ / path_.nSamples;
      ...
      for (unsigned i = 0; i < path_.nSamples; ++i)
      {
        ...
        path_.pose.p_z[i] = mp_param_.pz[i] - deltaZpersample * i;
    ...
    std::cout << "[MtcMPIC::initialize]: Initialized" << std::endl;
    this->print();
    return true;
  }

  void MtcMPIC::linearPath(const double s)
  {
  ...
  void MtcMPIC::linearPathGradient(const double s)
  {
    lrt_mpic::selectIndexOnPath(index_, path_.s, s);
  ...
  void MtcMPIC::rePlanPath(const double s, const Eigen::VectorXd &q, const Eigen::VectorXd &p, const Eigen::VectorXd &F)
  {
    if (wghts_.Q_F(2, 2) > 0)
    {
      for (unsigned i = 0; i < path_.nSamples; ++i)
      {
        path_.pose.p_z[i] = p[2];
      }
    }
  }
